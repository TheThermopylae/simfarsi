<template>
  <article class="bg-white p-3 rounded-2xl">
    <div class="flex justify-between items-center mb-2">
      <h4>
        مبلغ سفارش :
        <span class="font-peydaB">{{ props.data.price.toLocaleString() }} تومان</span>
      </h4>
      <svg
        v-if="!loading"
        @click="changeStatus"
    ...
    <section
      class="grid gap-2 mt-3 text-2sm text-center"
      :class="{'grid-cols-2' : props.data.products.length > 1}"
    >
      <div
